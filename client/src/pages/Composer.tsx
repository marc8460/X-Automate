...
import {
  ...
  useMediaItems, useGenerateTweets, usePostNow, useExtensionStatus,
} from "@/lib/hooks";
...
import { PostViaExtension } from "@/lib/extensionBridge";
...
export default function Composer() {
  ...
  const [isPostingExtension, setIsPostingExtension] = useState(false);
  ...
  const handlePostWithExtension = async () => {
    if (!draftText.trim()) return;
    if (draftText.length > charLimit) {
      toast({ title: "Too long", description: `Max ${charLimit} characters for this platform.`, variant: "destructive" });
      return;
              ...
              <div className="flex gap-2">
                {isExtensionConnected && composerTab !== "threads" && (
                  <Button
                    ...
                    data-testid="button-post-extension"
                  >
                    {isPostingExtension ? (
