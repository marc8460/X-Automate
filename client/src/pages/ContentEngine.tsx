...
import {
  ...
  X, Plus, Loader2, Wand2, Trash2, Clock, Zap,
} from "lucide-react";
import {
...
export default function ContentEngine() {
        ...
        { style: selectedStyle, topic, seductiveness, imageUrl: imageUrl || undefined },
        {
          onSuccess: (data) => {
            ...
            toast({
              ...
              description: `${(data.tweets || []).length} caption ideas for your photo.`,
            });
          },
          onError: (err: any) =>
  ...
  const handleSaveDraft = async () => {
    ...
    toast({
      title: "Saved",
      description: draftSchedule ? `Tweet scheduled for ${formatScheduleDisplay(draftSchedule)}.` : "Tweet added to queue.",
    });
  };
  ...
  // Currently scheduled queue card
  const queueTweetForModal = queueModalForId
    ? tweets.find((t) => t.id === queueModalForId)
    : null;

  return (
    <div className="space-y-8 pb-12">
      <div>
        <h1 className="text-3xl font-bold font-display tracking-tight" data-testid="text-content-title">
          Content Engine
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 pt-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-primary font-mono" data-testid="text-seductiveness-value">
                    {seductiveness}%
                  </span>
                </div>
                <Slider
          ...
          <Card className="p-6 glass-panel border-border/30">
            ...
            {attachedImage && (
              <div className="relative mb-4 inline-block">
                <img
                  ...
                  data-testid="img-draft-attached"
                />
                <button
            ...
            {/* Action buttons */}
            <div className="flex items-center justify-between">
              <span className={`text-xs ${draftText.length > 280 ? "text-red-400 font-medium" : "text-muted-foreground"}`}>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  data-testid="button-post-now-draft"
                >
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge
                            ...
                            data-testid={`badge-tweet-style-${tweet.id}`}
                          >
                            {tweet.style}
                          </Badge>
                          ...
                          {tweet.status === "queued" && (
                            <Badge
                              variant="outline"
                              className="text-[10px] uppercase tracking-wider text-accent border-accent/30"
                              data-testid={`status-queued-${tweet.id}`}
                            >
                              In Queue
                            </Badge>
                          )}
                          ...
                          {tweet.scheduledAt && (
                            <span
                              ...
                              data-testid={`text-schedule-${tweet.id}`}
                            >
                      ...
                      <div className="flex gap-4">
                        <div className="flex-1">
                          <p
                            ...
                            data-testid={`text-tweet-content-${tweet.id}`}
                          >
                            {tweet.text}
                          </p>
                        </div>
                        {tweet.imageUrl && (
                          <div className="relative w-24 h-24 rounded-lg overflow-hidden border border-border/50 shrink-0">
                            <img
                              ...
                              data-testid={`img-tweet-media-${tweet.id}`}
                            />
                            <button
                        ...
                        <div className="ml-auto flex gap-2">
                          ...
                          {tweet.status === "failed" && (
                            <Button
                              ...
                              disabled={postingNowId === tweet.id}
                              onClick={() => {
                                updateTweetMutation.mutate({ id: tweet.id, status: "queued" });
                              }}
                            >
        ...
        <div className="space-y-6">
          <Card className="p-5 glass-panel bg-secondary/10">
            <h3 className="font-display font-medium mb-2 text-sm uppercase tracking-wider text-muted-foreground">
              Queue Status
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-border/50 pb-2">
                <span className="text-sm text-muted-foreground">Scheduled Today</span>
                <span className="font-mono text-primary font-medium" data-testid="text-scheduled-today">
                  {tweets.filter((t) => t.status === "queued").length}/5
                </span>
              </div>
              <div className="flex justify-between items-center border-b border-border/50 pb-2">
                <span className="text-sm text-muted-foreground">Generated Library</span>
                <span className="font-mono font-medium" data-testid="text-library-count">
                  {tweets.length}
                </span>
              </div>
              <div className="flex justify-between items-center">
          ...
          <Card className="p-5 glass-panel border-accent/20">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Inappropriate terms are currently being filtered. The system will rely on suggestion,
              soft dominance, and playful teasing to maintain a professional yet engaging presence.
            </p>
          </Card>
        </div>
      </div>

      {/* Draft editor schedule modal */}
      <DateTimeWheelModal
        open={showDraftModal}
